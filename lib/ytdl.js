...
function INFOLOG(info) {
    return console.log(color('[ INF ]'), color(time), color(info))
}
        ...
        if (ytIdRegex.test(url)) {
            ...
            post('https://www.y2mate.com/mates/en60/analyze/ajax', {
                ...
                .then(res => {
                    ...
                    ansx = Object.fromEntries([...document.querySelectorAll('td > a[href="#"]')].filter(v => !/\.3gp/.test(v.innerHTML)).map(v => [v.innerHTML.match(/.*?(?=\()/)[0].trim(), v.parentElement.nextSibling.nextSibling.innerHTML]))
                    filesize = ansx["360p"]
        ...
        if (ytIdRegex.test(url)) {
            ...
            post('https://www.y2mate.com/mates/en60/analyze/ajax', {
                ...
                .then(res => {
                    ...
                    let aww = Object.fromEntries([...document.querySelectorAll('td > a[href="#"]')].filter(v => !/\.3gp/.test(v.innerHTML)).map(v => [v.innerHTML.match(/.*?(?=\()/)[0].trim(), v.parentElement.nextSibling.nextSibling.innerHTML]))
