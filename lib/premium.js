...
const addPremiumUser = (userId, expired, _dir) => {
    ...
    _dir.push(obj)
    fs.writeFileSync('./database/limit/premium.json', JSON.stringify(_dir, null, 2))
}
...
const expiredCheck = (fadly, _dir) => {
    setInterval(() => {
        ...
        if (position !== null) {
            ...
            let txt = `Premium Expired, Terimakasih Sudah Berlangganan Di Chitanda Eru Bot`
            fadly.sendMessage(_dir[position].id, { text: txt })
